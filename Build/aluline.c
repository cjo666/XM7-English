 ...
 *	  2001.11.22		直線補間アルゴリズムを変更
 *	  2002.01.06		ラインスタイルパターンレジスタの内容を点を打つごとに
 *						ローテートするように変更
 *						直線補間時にマスクレジスタを保護しないように変更
 *	  2002.03.13		直線補間部がBUSYになっている時間が描画したドット数に
 *						合わせて変化するように変更
 *	  2003.04.06		直線補間全力モードの動作を変更
 *	  2003.06.19		BUSY時間の計算を1ドット単位から1バイト単位に変更
 *						(そのほうが実行時間が実機に近いような気がする…)
 *	  2004.03.17		無意味にコメント増殖(ヴァク
 *						変数名を整理
 *	  2004.09.21		コマンド1の挙動を黒色書き込みに修正
 *	  2010.07.14		alu_disableの初期値を実機に合わせ0x0fに変更
 *	  2012.05.04		DEATH FORCEのメーカーロゴで不具合が発生するので
 *						alu_disableの初期値を0x00に戻す
 */
