 ...
 *
 *	RHG履歴
 *	  2001.11.19		リセット時・ステートファイルロード時等のキーオフが
 *						FM音源ディスプレイに反映されない問題を修正
 *	  2002.03.22		タイマのイベント設定タイミングを修正
 *	  2002.06.15		FM-7モード時の独立PSGエミュレーション対応に伴ってPSG
 *						I/Oアクセス時のバージョンチェックを追加
 *	  2002.06.29		プリスケーラ設定時にコマンドレジスタ→データレジスタの
 *						順でアクセスすると正しく設定が行われない問題を修正
 *	  2002.08.18		FM-7モード時のOPN無効化機能を追加
 *	  2002.08.23		OPN無効化機能関係部を少し書き直し
 *	  2003.06.19		タイマの挙動を一部修正
 *	  2003.07.18		タイマの挙動を再修正
 *	  2004.04.22		タイマの挙動を再々修正(滅
 *	  2004.08.13		ソース管理効率向上のためOPN系のソースを統合
 *						(代わりに見た目がやや汚くなった…)
 *	  2005.05.13		PSGリード時のビットマスクを追加 (from MS×PLAYer...)
 *	  2010.06.18		FM-8モード時に標準OPNが有効だった問題を修正
 *	  2012.04.20		一応OPN使用フラグを実装。現状では特に意味はありません
 *						OPN有効フラグ/OPN使用フラグをステートデータに保存/復帰
 *						するようにした。WHG/THGとデータ構造を合わせるためにイ
 *						レギュラーな順番となっています。
 */
...
#if XM7_VER == 1
BOOL fmx_flag;							/* FM-X PSGモードフラグ */
BOOL fmx_use;							/* FM-X PSG 使用フラグ */
#endif
