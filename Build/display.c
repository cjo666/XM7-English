 ...
 *
 *	RHG履歴
 *	  2001.07.23(爆)	画面モードの扱いを変更
 *	  2001.07.27		不要な再描画のために速度が低下する問題を修正
 *	  2001.07.29		400ラインモード時のポインタ設定値がおかしい問題を修正
 *	  2001.11.10		400ラインモード時のスクロール挙動を修正(爆)
 *	  2001.11.19		VRAMの配置を実機に合わせたことに伴う変更多数(^^;
 *	  2001.11.25		ハードウェアウィンドウの挙動を変更
 *	  2002.03.06		FM-7モード時も$D430への書き込みが有効になる問題を修正
 *	  2002.05.25		V2コンパイル時に不要なコードを生成しないように修正
 *						4096色・26万色のスクロール処理を統合
 *	  2002.11.01		C版memcpy400lを追加
 *	  2003.02.20		200ラインモードと400ライン/4096色モードで別になってい
 *						たVRAM表示ポインタ(表・裏)を統合
 *	  2003.03.05		画面モードが200ライン8色モード以外の状態でサブモニタを
 *						切り替えると画面がおかしくなることがある問題を修正
 *	  2003.03.29		ハードウェアウィンドウレジスタ書き込み部を変更
 *						ハードウェアウィンドウレジスタの有効ビット数を変更
 *	  2003.06.02		CLR命令でBUSYフラグを落とすとデッドロックする問題に対
 *						処
 *	  2003.11.20		XM7 V1.1対応
 *						FM-7モード時にNMIマスクがかかる可能性があった問題を修
 *						正
 *	  2003.11.21		$FD04 bit5(漢字ROM接続の切り換え機能)に対応
 *	  2004.01.24		リセット時に直前のVRAMオフセットにしたがってスクロール
 *						を行うように変更
 *	  2004.11.20		vsync_notifyの呼び出しを表示期間の終了時に変更
 *	  2010.01.13		TrueColor時の輝度変換テーブルを追加
 *	  2010.12.09		FM77AV40/20系から抽出したJIS83準拠漢字ROMデータにおい
 *						て、FM-77モードのJIS78準拠エミュレーションを追加
 *	  2011.04.09		FM-77モードで漢字ROMアドレス計算が間違っていたのを修正
 *	  2011.05.30		動作モード別にJIS78準拠フォントとJIS83準拠フォントの同
 *						居ができるよう変更
 *	  2012.05.29		ステートロード時に400ラインカード存在フラグと画面モー
 *						ドの整合性チェックを行うようにした
 *	  2012.08.15		ハードウェアウィンドウレジスタの処理時にFALSEを返して
 *						いた問題を修正
