 ...
 *
 *	RHG履歴
 *	  2001.11.19		ステートファイルロード後にキーリピートする問題を修正
 *	  2002.02.06		コード系変更時の処理を改善
 *	  2002.07.17		SHIFTキー押下状態フラグを左右で内部的に独立させた
 *						SHIFTキーが左右両方押された状態から片方だけ離した場合に
 *						SHIFT状態が解除されないように変更
 *	  2002.08.04		スキャンモード以外でCAP/かなキーが効かない問題を修正
 *	  2003.05.05		リセット後、初めてコードフォーマットを変更したときに不
 *						正なキーコードが発行されることがある問題を修正
 *	  2003.09.30		CTRL+@が入力できない問題を修正
...
BYTE amnork_mode;				/* アムノーク隠し技用 */
BOOL keyenc_beep_flag;			/* キーエンコーダ単音BEEPフラグ */
#else
#if !defined(XM7PURE)
BOOL key_mode_flag;				/* FM-8モード時のFM-7互換キー入力モードフラグ */
WORD key_fm8;					/* キーコード(FM-8) */
BYTE key_push_data[16];			/* キー入力状態 */
#endif
#endif
...
BOOL key_hidden_message_flag;	/* キーエンコーダ 隠しメッセージ実行済フラグ */
BOOL key_hidden_message_run;	/* キーエンコーダ 隠しメッセージ実行中フラグ */
BYTE key_hidden_message_count;	/* キーエンコーダ 隠しメッセージカウンタ */
#endif
 ...
 *	物理コード、SHIFTなし、SHIFTありの順
 */
#define	ctrl_fm8_table	(ctrl_key_table)

/*
 *	GRAPHモード（FM-8モード時）
 *	物理コード、SHIFTなし、SHIFTありの順
 */
WORD graph_fm8_table[] = {
	0x5d, 0x0101, 0xffff,
	0x5e, 0x0102, 0xffff,
	0x5f, 0x0103, 0xffff,
	0x60, 0x0104, 0xffff,
 ...
 *	物理コード、SHIFTなし、SHIFTありの順
 */
#define	kana_fm8_table	(kana_key_table)
...
#elif !defined(XM7PURE)
	key_mode_flag = FALSE;
	memset(key_push_data, 0, sizeof(key_push_data));
#endif
...
void FASTCALL keyboard_reset(void)
{
	...
	/* 隠しメッセージ */
	key_hidden_message_flag = FALSE;
	key_hidden_message_run = FALSE;
	key_hidden_message_count = 0;
#elif !defined(XM7PURE)
	key_fm8 = 0xffff;
#endif
}
 ...
 *	ACK信号送信
 */
static BOOL FASTCALL key_send_ack(void)
{
	key_trans_ack = TRUE;
 ...
 *	シフト状態フラグ更新
 */
static BOOL FASTCALL keyboard_update_shift(BYTE dat)
{
	switch (dat) {
		/* 左SHIFT 押した */
		case 0x53:
		...
		case 0x5c:
#if XM7_VER >= 2
		/* スキャンモード時、かつBREAKキーと他のキーが同時押しされた場合の細工
		   (アムノーク) */
			if (key_format == KEY_FORMAT_SCAN) {
				amnork_mode = 1;
			}
...
WORD FASTCALL keyboard_to_scan(BYTE dat)
{
	...
	/* (アムノーク) */
	if (key_format == KEY_FORMAT_SCAN) {
		if (((dat & 0x7f) != 0x5c) && (amnork_mode == 1)) {
			break_flag = FALSE;
			maincpu_firq();

			amnork_mode = 2;
			dat = (BYTE)((dat & 0x80) | 0x7f);
		}
	}
 ...
 *	BEEP音停止イベント
 */
static BOOL FASTCALL keyencbeep_event(void)
{
	keyenc_beep_flag = FALSE;
	keyencbeep_notify();
	schedule_delevent(EVENT_KEYENC_BEEP);
	return TRUE;
}
 ...
 *	BEEP音発生
 */
static void FASTCALL keyenc_beep(void)
{
	keyenc_beep_flag = TRUE;
	keyencbeep_notify();
	schedule_setevent(EVENT_KEYENC_BEEP, 25000, keyencbeep_event);
}
 ...
 *	隠しメッセージ入力
 */
static BOOL FASTCALL keyenc_hidden_message_event(void)
{
	key_fm7 = key_hidden_message[key_hidden_message_count];
	key_hidden_message_count ++;
	...
	/* 終了判定 */
	if (key_fm7 == 0x00) {
		schedule_delevent(EVENT_KEYENC_MESSAGE);
		key_hidden_message_flag = TRUE;
		key_hidden_message_run = FALSE;
...
/*
 *	キーボード
 *	Make/Breakコード→FM-8コード変換
 */
WORD FASTCALL keyboard_to_fm8(BYTE dat)
{
	int i;
	...
	/* CTRLモード */
	if (ctrl_flag) {
		for (i=0; i<(sizeof(ctrl_fm8_table)/3); i++) {
			if (dat == ctrl_fm8_table[i * 3]) {
				if (!shift_flag) {
					return ctrl_fm8_table[i * 3 + 1];
				}
				else {
					return ctrl_fm8_table[i * 3 + 2];
				}
			}
		}
		return 0xffff;
	...
	/* GRAPHモード */
	if (graph_flag) {
		for (i=0; i<(sizeof(graph_fm8_table)/3); i++) {
			if (dat == graph_fm8_table[i * 3]) {
				if (!shift_flag) {
					return graph_fm8_table[i * 3 + 1];
				}
				else {
					return graph_fm8_table[i * 3 + 2];
				}
			}
		}
		return 0xffff;
	...
	/* かなモード */
	if (kana_flag) {
		for (i=0; i<(sizeof(kana_fm8_table)/3); i++) {
			if (dat == kana_fm8_table[i * 3]) {
				if (!shift_flag) {
					return kana_fm8_table[i * 3 + 1];
				}
				else {
					return kana_fm8_table[i * 3 + 2];
				}
			}
		}
		return 0xffff;
	...
	/* CAPモード */
	if (caps_flag) {
		for (i=0; i<(sizeof(caps_fm8_table)/3); i++) {
			if (dat == caps_fm8_table[i * 3]) {
				if (!shift_flag) {
					return caps_fm8_table[i * 3 + 1];
				}
				else {
					return caps_fm8_table[i * 3 + 2];
				}
			}
		}
		return 0xffff;
	...
	/* ノーマルモード */
	for (i=0; i<(sizeof(norm_fm8_table)/3); i++) {
		if (dat == norm_fm8_table[i * 3]) {
			if (!shift_flag) {
				return norm_fm8_table[i * 3 + 1];
			}
			else {
				return norm_fm8_table[i * 3 + 2];
			}
		}
	}
 ...
 *	リピートタイマイベント(FM-8)
 */
static BOOL FASTCALL keyboard_event_fm8(void)
{
	if (key_repeat_flag) {
		/* ここでキーコードを入れ替える */
		if (key_fm8 != 0xffff) {
...
#if !defined(XM7PURE)
	/* キー状態保持 */
	key_push_data[dat >> 3] |= (0x80 >> (dat & 0x07));
#endif
#endif
...
#if XM7_VER >= 2
			if (fm7_ver >= 2) {
				if (lshift_flag && rshift_flag && caps_flag && kana_flag &&
					graph_flag && (dat == 0x15)) {
					if (!key_hidden_message_flag) {
						key_hidden_message_run = TRUE;
						schedule_setevent(EVENT_KEYENC_MESSAGE, 120000,
							keyenc_hidden_message_event);
					}
					else {
						keyenc_beep();
					}
...
#if XM7_VER == 1
#if !defined(XM7PURE)
	if (!key_mode_flag && (fm_subtype == FMSUB_FM8)) {
		n = 0;

		/* 有効キー押下数検出 */
		for (num = 0x01; num <= 0x66; num ++) {
			/* シフト系キーはカウントしない */
			if (((num >= 0x52) && (num <= 0x56)) ||
				(num == 0x5a) || (num == 0x5c)) {
				continue;
			}
			if (key_push_data[num >> 3] & (0x80 >> (num & 0x07))) {
				n ++;
				key_dat = (BYTE)num;
			}
		}
		...
		/* 単一キー入力なら */
		if (n == 1) {
			code = keyboard_to_fm8(key_dat);

			/* リピート中なら */
			if ((event[EVENT_KEYBOARD].flag != EVENT_NOTUSE) &&
				(event[EVENT_KEYBOARD].reload == key_repeat_time2)) {
				schedule_delevent(EVENT_KEYBOARD);
				key_fm8 = code;
				key_repeat_code = key_dat;
				schedule_setevent(EVENT_KEYBOARD, key_repeat_time1, keyboard_event_fm8);
				return;
			}
		}
		else {
...
void FASTCALL keyboard_break(BYTE dat)
{
	...
	if (key_format == KEY_FORMAT_SCAN) {
#if XM7_VER >= 2
		/* BREAKキーと他のキーが同時押しされた場合の細工 (アムノーク) */
		if (((key_scan & 0x7f) != 0x5c) && (amnork_mode == 2)) {
			amnork_mode = 0;
			key_fm7 = 0xff;
			keyenc_beep();
		}
...
#if !defined(XM7PURE)
	/* リピート状態を待避 */
	rep_flag = event[EVENT_KEYBOARD].flag;
#endif
#endif
	...
	if (fm_subtype == FMSUB_FM8) {
		...
		for (num = 0x01; num <= 0x66; num ++) {
			/* シフト系キーはカウントしない */
			if (key_push_data[num >> 3] & (128 >> (num & 0x07))) {
				if (((num >= 0x52) && (num <= 0x56)) ||
					(num == 0x5a) || (num == 0x5c)) {
					continue;
				}
		...
		if (n == 1) {
			...
			if (code != 0xffff) {
				...
				if ((rep_flag != EVENT_NOTUSE) &&
					...
					if ((key_fm7 >= 0x101) && (key_fm7 <= 0x10a)) {
						/* PFキーは、リピートなし */
						schedule_delevent(EVENT_KEYBOARD);
					}
					else {
...
BOOL FASTCALL keyboard_save(int fileh)
{
	...
	/* キーボードエンコーダ隠しメッセージ(Ver9.19/7.19で拡張) */
	if (!file_bool_write(fileh, key_hidden_message_flag)) {
		return FALSE;
	}
	if (!file_bool_write(fileh, key_hidden_message_run)) {
		return FALSE;
	}
	if (!file_byte_write(fileh, key_hidden_message_count)) {
		return FALSE;
	}
#else
#if defined(XM7PURE)
	...
	if (!file_word_write(fileh, 0xffff)) {
		return FALSE;
	}
	if (!file_write(fileh, tmp, sizeof(tmp))) {
		return FALSE;
	}
#else
	if (!file_word_write(fileh, key_fm8)) {
		return FALSE;
	}
	if (!file_write(fileh, key_push_data, sizeof(key_push_data))) {
		return FALSE;
	}
#endif
#endif
...
BOOL FASTCALL keyboard_load(int fileh, int ver)
{
	...
	/* キーボードエンコーダ隠しメッセージ(Ver9.19/7.19で拡張) */
#if XM7_VER >= 3
	if ((ver >= 919) || ((ver >= 719) && (ver <= 799))) {
#else
	if ((ver >= 719) && (ver <= 799)) {
#endif
		if (!file_bool_read(fileh, &key_hidden_message_flag)) {
			return FALSE;
		}
		if (!file_bool_read(fileh, &key_hidden_message_run)) {
			return FALSE;
		}
		if (!file_byte_read(fileh, &key_hidden_message_count)) {
			return FALSE;
		}
		if (fm7_ver == 1) {
			key_hidden_message_flag = FALSE;
	...
	if (ver >= 310) {
		...
		if (!file_read(fileh, tmp, sizeof(tmp))) {
			return FALSE;
		}
#else
		if (!file_word_read(fileh, &key_fm8)) {
			return FALSE;
		}
		if (!file_read(fileh, key_push_data, sizeof(key_push_data))) {
			return FALSE;
		}
#endif
	}
	...
	schedule_handle(EVENT_KEYENC_MESSAGE, keyenc_hidden_message_event);
#endif
