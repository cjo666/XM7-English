 ...
 *
 *	RHG履歴
 *	  2001.07.23(爆)	V2でのVRAM用メモリ確保量を96KBに戻した
 *	  2001.11.05		V2で$D800～$FFFFにライトアクセスするとメモリを破壊する
 *						問題を修正
 *	  2003.11.20		XM7 V1.1対応(しゃがみ大パンツ)
 *	  2007.01.13		V1.1の400ラインテキストフォントを40EX用拡張ROMイメージ
 *						で代用できるように変更(大多数には関係なし)
 *	  2010.06.09		実機通りI/O領域のミラー領域を反映させるように変更
 *	  2010.09.29		FM-8のROMイメージしかない場合にドライブ番号/FM音源ディ
 *						スプレイの文字表示が正常でない問題に対処
 *	  2012.05.29		ステートロード時の400ラインカードの存在チェックを廃止
 *	  2017.05.05		FM-8/7モード時にVRAMアクセスフラグを立てずにブランキン
 *						グ期間外にVRAMアクセスした場合、無効となるように挙動を
 *						変更
 */
 ...
 *	グローバル ワーク
 */
BYTE *vram_c;						/* VRAM(タイプC) $C000(裏バンク有り) */
...
#if XM7_VER >= 2
BYTE *vram_b;						/* VRAM(タイプB) $C000(裏バンク有り) */
