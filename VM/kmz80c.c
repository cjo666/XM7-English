...
const static Uint8 flagtable[0x200] = {
/*
  0 <= i <= 0x1ff
  CF(bit0) (i > 0xff)
  NF(bit1) always(1)
  PF(bit2) parity(i & 0xff)
  XF(bit3) !!(i & 0x008)
  HF(bit4) always(1)
  YF(bit5) !!(i & 0x020)
  ZF(bit6) !(i & 0x0ff)
...
{ \
	...
	tmp = BC + 0xffff; C = RTO8(tmp); B = RTO8(tmp >> 8); \
...
{ \
	...
	tmp = (dir & ((0xa492 ^ 0x6da4) << 2)) ^ (0xa492 << 2); \
	/* tmp = ((dir) == 1) ? (0xa492 << 2) : (0x6da4 << 2); */ \
	tmp >>= (((C & 3) << 2) + (TOP & 3)); \
	tmp ^= FLAGTBL(B) & PF; \
	if (B & 0x0F) \
		tmp ^= (B << 2) | ((~B << 1) & B); \
...
#define M_POSTINX(dir) M_POSTINX_(dir)
#define M_POSTOTX(dir) M_POSTOTX_(dir)
...
static void M_LDX_FUNC(KMZ80_CONTEXT *context, Uint32 dir) M_LDX_(dir)
static void M_CPX_FUNC(KMZ80_CONTEXT *context, Uint32 dir) M_CPX_(dir)
static void M_INX_FUNC(KMZ80_CONTEXT *context) M_INX_(d)
static void M_OUTX_FUNC(KMZ80_CONTEXT *context) M_OUTX_(d)
static void M_POSTINX_FUNC(KMZ80_CONTEXT *context, Uint32 dir) M_POSTINX_(dir)
static void M_POSTOTX_FUNC(KMZ80_CONTEXT *context, Uint32 dir) M_POSTOTX_(dir)
static void M_OTXM_FUNC(KMZ80_CONTEXT *context) M_OTXM_(d)
static void M_POSTOTXM_FUNC(KMZ80_CONTEXT *context, Uint32 dir) M_POSTOTXM_(dir)
...
#define M_POSTINX(dir) M_POSTINX_FUNC(context, dir)
#define M_POSTOTX(dir) M_POSTOTX_FUNC(context, dir)
		...
		do
		{
			...
			switch (opp->adr)
			{
				...
				case ADR_ID:
					if (STATE & ST_DD)
					{
						Uint32 tmp;
						FETCH(tmp);
						TAD = TDX = RTO16(IX + (tmp ^ 0x80) + 0xff80);
					}
					else if (STATE & ST_FD)
					{
						Uint32 tmp;
						FETCH(tmp);
						TAD = TDX = RTO16(IY + (tmp ^ 0x80) + 0xff80);
					}
					else
						TAD = HL;
					break;
			...
			switch (opp->op)
			{
				...
				case OP_JPCX:
					...
					break;
				case OP_JR:
					TPC = TDX = RTO16(TPC + (TOP ^ 0x80) + 0xff80);
					break;
				case OP_JRCC:
					if (!CHECKCC((op0 >> 3) & 3)) break;
					CYCLE += CYT[0x504];
