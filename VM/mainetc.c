 ...
 *
 *	RHG履歴
 *	  2003.05.28		BEEP ONの状態でステートファイルを読み込んでもBEEPが止
 *						まらない問題を修正
 *	  2003.08.12		スピーカフラグの初期化を忘れていたのを修正(汁
 *	  2005.11.12		FM-8モード時にタイマ割込を停止するように変更
 *	  2010.07.28		FM-8モード時にも$FD0FのR/WでBASIC ROM/裏RAMの選択がで
 *						きるように変更
 */
...
#if defined(MOUSE)
BOOL ptm_irq_flag;					/* PTM割り込み フラグ */
BOOL ptm_irq_mask;					/* PTM割り込み マスク */
#endif
...
#if XM7_VER == 1
BYTE banksel_en;					/* バンク切り換えイネーブルフラグ */
#endif
 ...
 *	プロトタイプ宣言
 */
static BOOL FASTCALL mainetc_event(void);	/* 2.03ms タイマーイベント */
