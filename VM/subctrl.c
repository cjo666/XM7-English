 ...
 *
 *	RHG履歴
 *	  2001.07.27		display_setpointer(旧display_setparam)の仕様変更に伴う
 *						変更
 *	  2001.08.14		BUSY信号の遅延を行うように変更
 *	  2001.08.27		画面モード切り替え時の不要な再描画を抑制
 *	  2001.09.02		サブバンク切り替え処理にALUのリセットを追加
 *	  2001.11.24		サブHALTタイミングを変更
 *	  2001.04.01		ステートファイルVer7.01のロードに失敗する問題を修正
 *	  2002.12.04		ホットリセット用の細工を追加
 *	  2003.03.29		ステートロード時のサブI/Fレジスタの復帰に対応
 *	  2003.06.02		CLR命令でBUSYフラグを落とすとデッドロックする問題に対処
...
BOOL extdet_disable;					/* EXTDETディセーブル */
...
BOOL submode_fix;						/* FM77AV用OS-9対策フラグ */
#endif
#endif
