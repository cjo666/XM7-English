 ...
 *
 *	RHG履歴
 *	  2001.05.08	内部レジスタの４チャンネル対応化/データチェイン対応
 *					HALTサイクルスチール転送時のクロック引き延ばし動作を追加
 *					転送中にBCRを0にした場合の挙動を修正
 *	  2001.05.09	ステートファイル書き込み処理の新機能対応(バージョン9専用)
 *	  2001.11.15	転送終了してもBUSYフラグが落ちない問題を修正
 *					転送終了時にTxRQビットが勝手に落ちる問題を修正
 *					DENDビットがリードアクセスでクリアされるように修正
 *					その他、細かい部分の挙動を実機に合わせて修正
 *					AV40仕様対応版とソースを統合
 *	  2002.02.12	BCR書き換えによる中断時にBUSYフラグが落ちない問題を修正
 *	  2002.02.16	BCR書き換えによる中断時にDENDフラグが立たない問題を修正
 *	  2002.02.19	IRQがかかりっぱなしになる問題を修正(………)
 *	  2002.05.09	DMA転送完了フラグを新設
 *	  2002.05.11	FDCコマンド発行から100μs後にDMA転送開始するように変更
 *	  2002.07.17	割り込み制御レジスタ bit7の挙動を修正
 *	  2002.12.09	FDCコマンド発行から転送開始までのウェイトを50μsに変更
 *	  2004.08.16	バースト転送の実装を少しまともにした
 */
...
BOOL FASTCALL dmac_save(int fileh)
{
	...
	/* Ver9拡張・AV40仕様時 (40EX仕様時との互換性がなくなるので注意) */
#if defined(DMAC_AV40)
	for (i=1;i<4;i++) {
