 ...
 *
 *	RHG履歴
 *	  2001.10.03		縮小画像キャプチャ機能を追加
 *	  2002.03.04		8色モードのパレット情報がおかしくなる問題を修正
 *	  2002.03.27		VTP→T77変換が使いものにならない問題を修正
 *	  2002.05.04		F-BASICユーザディスク作成機能を追加
 *	  2002.09.09		VTP→変換のGAP出力部をVC++で警告が出ないように変更
 *	  2002.10.27		8色モード画像縮小の混色アルゴリズムを変更
 *	  2003.08.12		ユーザディスクのIPLにORCC #$50を追加
 *	  2004.03.17		2D→D77変換処理を2DDベタイメージに対応
 *						2D→D77変換のメディアタイプ書き込みを明示的にした
 *	  2004.12.15		77L4 400ライン画像キャプチャを追加
 *	  2004.12.22		77L4 400ライン縮小画像キャプチャをインテンシティビ
 *						ット対応に変更
 *	  2010.01.13		TrueColor時の輝度変換をテーブル化
 *	  2012.07.01		ブランクバブルカセット作成機能をXM7dashから移植
 *	  2014.03.28		画面キャプチャ機能をグリーンモニタモード/TTLモニタ
 *						モードに対応
 */
 ...
 *	画像縮小用
 */
#define GAMMA200L	1.483239697419133	/* 画像縮小時のγ補正値 (200LINE) */
#define GAMMA400L	1.217883285630907	/* 画像縮小時のγ補正値 (400LINE) */
...
#if XM7_VER >= 3
	switch (screen_mode) {
		case SCR_262144 :	/* 262144色 ファイルサイズ 14+40+768000 */
							filehdr[2] = 0x36;
							filehdr[3] = 0xb8;
							filehdr[4] = 0x0b;
							break;
		case SCR_4096	:	/* 4096色 ファイルサイズ 14+40+512000 */
							filehdr[2] = 0x36;
							filehdr[3] = 0xd0;
							filehdr[4] = 0x07;
							break;
		default		:	/* 640x200/400 8色 ファイルサイズ 14+40+16*4+128000 */
							filehdr[2] = 0x76;
							filehdr[3] = 0xf4;
							filehdr[4] = 0x01;
	}
...
#if XM7_VER == 1
/*
 *  BMPデータ書き込み(640モード,モノクロ疑似400ライン)
 */
...
/*
 *  BMPデータ書き込み(640モード・カラー疑似400ライン・縮小画像)
 */
#if XM7_VER >= 2
