...
; FM-7 EMULATOR "XM7"
;
; Copyright (C) 1999-2017 ＰＩ．(yasushi@tanaka.net)
; Copyright (C) 2001-2017 Ryu Takegami
;
; [ 6809CPU(x86アセンブラ版) ]
;
; 2004/09/21	割り込み時のサイクル数加算を忘れていたのを修正
; 2004/11/10	IRQ/FIRQ処理時に割り込みフラグが勝手にクリアされる問題を修正
; 2005/02/27	keisuke氏に指摘された部分を修正
;				(インデックス未定義ポストバイト,スタック退避/復帰処理ほか)
; 2006/06/04	プリバイト付きのLBRA($10 $20)/LBSR($10 $8D)に対応
; 2008/01/27	プリバイト付きLBRA/LBSRが正常に動作しない問題を修正
; 2010/07/24	SYNC/CWAI/ハードウェア割り込み部分の動作を修正(Thanks>Toma)
;				(↑はXM7pureでは基本的に無効になります)
;				一部コメントアウトしただけだった未使用部分を削除
; 2010/07/25	HALT命令まわりのラベル・コメントを修正
;				CWAIフラグが立っている時に割り込みがかかると既に次命令のPCは
;				PUSHされているにも関わらず無意味なPC更新が発生するのを修正
; 2011/12/12	ROR命令でVフラグがクリアされる問題を修正
; 2015/07/08	HALT命令の名称をHCF命令に変更
; 2015/08/05	CWAI待ち状態で割り込みが入った際の実行サイクル数が過剰に加算さ
;				れる問題を修正
