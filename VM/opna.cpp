...
void OPNBase::SetPrescaler(uint p)
{
	...
	if (prescale != (uint8)p)
	{
		...
		if (interpolation)
		{
			...
			{
				rate >>= 1;
				mpratio = int(double(rate) * FM_IPSCALE / psgrate);
			} while (mpratio >= (FM_IPSCALE * 2));
		}
		else
		{
			rate = psgrate;
...
void OPNABase::FMMixM(Sample* buffer, int nsamples, int l, int r)
{
	if (fmvolume > 0)
	{
		// 準備
		...
		{
			if (interpolation)
				Mix6IM(buffer, nsamples, act, l, r);
			else
				Mix6M(buffer, nsamples, act, l, r);
		}
	}
}
...
{
	if (activech & 0x001) {wbuf[0] = ch[0].CalcL(); *dest[0]  = wbuf[0];}
	if (activech & 0x004) {wbuf[1] = ch[1].CalcL(); *dest[1] += wbuf[1];}
	if (activech & 0x010) {wbuf[2] = ch[2].CalcL(); *dest[2] += wbuf[2];}
	if (activech & 0x040) {wbuf[3] = ch[3].CalcL(); *dest[3] += wbuf[3];}
	if (activech & 0x100) {wbuf[4] = ch[4].CalcL(); *dest[4] += wbuf[4];}
	if (activech & 0x400) {wbuf[5] = ch[5].CalcL(); *dest[5] += wbuf[5];}
}

inline void OPNABase::MixSubS(int activech, ISample** dest)
{
	if (activech & 0x001) {wbuf[0] = ch[0].Calc(); *dest[0]  = wbuf[0];}
	if (activech & 0x004) {wbuf[1] = ch[1].Calc(); *dest[1] += wbuf[1];}
	if (activech & 0x010) {wbuf[2] = ch[2].Calc(); *dest[2] += wbuf[2];}
	if (activech & 0x040) {wbuf[3] = ch[3].Calc(); *dest[3] += wbuf[3];}
	if (activech & 0x100) {wbuf[4] = ch[4].Calc(); *dest[4] += wbuf[4];}
	if (activech & 0x400) {wbuf[5] = ch[5].Calc(); *dest[5] += wbuf[5];}
}
...
void OPNABase::Mix6(Sample* buffer, int nsamples, int activech)
{
	...
	for (Sample* dest = buffer; dest < limit; dest+=2)
	{
		...
		rcnt = (rcnt + 1) & 0x1ff;
		rbuf[0][rcnt] = wbuf[0] << (FM_ISHIFT + 3);
		rbuf[1][rcnt] = wbuf[1] << (FM_ISHIFT + 3);
		rbuf[2][rcnt] = wbuf[2] << (FM_ISHIFT + 3);
		rbuf[3][rcnt] = wbuf[3] << (FM_ISHIFT + 3);
		rbuf[4][rcnt] = wbuf[4] << (FM_ISHIFT + 3);
		rbuf[5][rcnt] = wbuf[5] << (FM_ISHIFT + 3);
...
void OPNABase::Mix6M(Sample* buffer, int nsamples, int activech, int l, int r)
{
	Sample* limit = buffer + nsamples * 2;
	// Mix
	ISample ibuf[4];
	ISample* idest[6];
	idest[0] = idest[1] = idest[2] = idest[3] = idest[4] = idest[5] = &ibuf[3];
	...
	for (Sample* dest = buffer; dest < limit; dest+=2)
	{
		...
		StoreSample(dest[0], (IStoSample(ibuf[3]) * l) >> 4);
		StoreSample(dest[1], (IStoSample(ibuf[3]) * r) >> 4);
	}
}
...
//	合成 - 線形補間
//
void OPNABase::Mix6IM(Sample* buffer, int nsamples, int activech, int l, int r)
{
	// Mix
	ISample ibuf[4];
	ISample* idest[6];
	...
	for (Sample* dest = buffer; dest < limit; dest+=2)
	{
		...
		StoreSample(dest[0], (INTERPOLATE(ml, delta) * l) >> 4);
		StoreSample(dest[1], (INTERPOLATE(mr, delta) * r) >> 4);
	}
	mixdelta = delta;
}
...
void OPNA::Mix(Sample* buffer, int nsamples)
{
	...
	psg.Mix2(buffer, nsamples, 16, 16);
...
//			nsamples	合成サンプル数
//
void OPNA::MixM(Sample* buffer, int nsamples, int l, int r)
{
	FMMixM(buffer, nsamples, l, r);
	psg.Mix2(buffer, nsamples, l, r);
	ADPCMBMix(buffer, nsamples);
	RhythmMix(buffer, nsamples);
}
...
//	初期化
//
bool OPN2::Init(uint c, uint r, bool ipflag)
{
	if (!SetRate(c, r, ipflag))
		return false;
	if (!OPNABase::Init(c, r, ipflag))
...
//		UpdateStatus(); //?
	...
	case 0x2a:	// YM2612 DAC OUT
		reg2a = (uint8)data;
		break;

	case 0x2b:	// YM2612 DAC ENABLE
		reg2b = (uint8)data;
		break;

	case 0x1b6:	// YM2612 DAC PANPOT
		dac_pan = (uint8)(data & 0xc0);
		OPNABase::SetReg(addr, data);		// thru
		break;
...
//	DAC 合成
//
void OPN2::DACMix(Sample* buffer, uint nsamples)
{
	uint i;
	ushort dat;

	for (i = 0; i < nsamples; i++) {
		dat = (ushort)((int8)(reg2a - 0x80) << 8);
		if (dac_pan & 0x80) {
			StoreSample(buffer[0], dat);
		}
		if (dac_pan & 0x40) {
			StoreSample(buffer[1], dat);
		}
		buffer += 2;
	}
}
