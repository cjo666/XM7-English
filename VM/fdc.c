 ...
 *
 *	RHG履歴
 *	  2001.12.03		ディスクイメージのマウント中はファイルをオープンしたま
 *						まの状態になるように変更 (towns版のみ)
 *						V2で2DDのディスクイメージがマウントできるのを修正
 *	  2001.12.09		2DDイメージを2Dモードで読むとトラック補正が行われない
 *						問題を修正
 *	  2002.02.09		セクタ毎のヘッダ情報のうち先頭の１バイト(トラック番号)
 *						がトラックデータに反映されない問題を修正 (Y.Sato)
 *	  2002.02.26		D77ファイルのセクタ検索処理でRECORD TYPEエラーとCRCエ
 *						ラーが同時に発生しない問題を修正 (Apollo)
 *	  2002.04.25		サイド情報の比較時にbit0のみ比較するように変更
 *	  2002.05.03		2DD切り換えを行った時にトラックの再読み込みを行うよう
 *						に変更
 *	  2002.05.06		トラックレジスタ・セクタレジスタをコマンド発行後に書き
 *						換えた時の挙動を修正
 *						READ ADDRESSコマンド実行後に読み出したIDフィールドのト
 *						ラック番号がセクタレジスタに入るように変更
 *	  2002.05.24		fdc_readsecでIDフィールドのサイド情報とサイドレジスタを
 *						比較をしないように変更
 *	  2002.11.10		ウェイト挿入・シーク音発生に対応
 *	  2002.12.16		WRITE TRACKコマンド実行中にFORCE INTERRUPTコマンドを発
 *						行して処理を打ち切った場合にもデータの書き込みを行うよ
 *						うに変更
 *	  2003.02.08		ウェイト動作時のシークコマンドでのFDC割り込みタイミン
 *						グを修正
 *	  2003.02.25		ウェイトなし動作時のシーク系コマンドでのIRQ On・FDC割
 *						り込みタイミングを修正
 *	  2003.03.15		Write Track時のDELETED DATA MARK生成に対応
 *						Read Track処理でデータ直前のSYNC/DATA ADDRESS MARKを
 *						入れ忘れていたのを修正
 *	  2003.04.02		Read Track中の状態がfdc_readyに反映されない問題を修正
 *	  2004.03.17		2DDベタイメージ対応
 *						2D/2DD/VFDファイルのトラック補正処理を修正
 *	  2004.03.23		D77ファイルの奇数トラック判定でミスっていたのを修正
 *	  2010.07.08		SEEKコマンド実行後のトラックレジスタにセットする値を
 *						データレジスタにセットした値に変更 (OS-9 L1 V1.2 L1.2)
 *	  2010.07.26		ブートフラグを廃止
 *	  2010.08.07		一時イジェクトされたメディアが存在する状態でカレント
 *						ドライブ以外にメディアがセットされた時に一時イジェク
 *						トが強制的に解除される問題を修正
 *	  2010.11.21		ファイル名の制限を256バイトまでに緩和
 *						メディアマウント時のファイル名サイズチェックを強化
 *	  2012.10.11		非カレントドライブにマウントしたD77ファイル内のメディ
 *						ア変更をした際に内部トラックバッファが書き換えられてし
 *						まう問題を修正
 *	  2017.11.28		V1/V2での2DDイメージアクセス(偶数トラックのみ)に対応
 */
...
BYTE fdc_medias[FDC_DRIVES];		/* メディア枚数 */
BYTE fdc_media[FDC_DRIVES];			/* メディアセレクト状態 */
...
static BYTE FASTCALL fdc_readsec(BYTE track, BYTE sector, BYTE side, BOOL sidecmp)
{
	...
	for (i=0; i<secs; i++) {
		...
		if (fdc_buffer[offset + 0x0008] == 0xb0) {
			/* 前に設定したステータスが無効になる問題を修正 (ｷﾎﾝｼﾞｼｮ) */
			stat |= FDC_ST_CRCERR;
		}
...
static void FASTCALL fdc_readbuf(int drive)
{
	...
	if (fdc_2ddmode) {
		...
		/* 2DDファイルを2D読みした場合、ここで補正 (AV40専用OS-9) */
...
BOOL FASTCALL fdc_setmedia(int drive, int index)
{
	...
	/* VFDファイルの場合、index = 0かをチェックしてヘッダ読み込み */
	if (fdc_ready[drive] == FDC_TYPE_VFD) {
		if (index != 0) {
			return FALSE;
		}
